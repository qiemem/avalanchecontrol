...
to-report stabilize
  ...
  while [any? active] [
    set fired (patch-set fired active)
    ask active [ 
      set sim-grains (sim-grains - 4)
      ask neighbors4 [ set sim-grains (sim-grains + 1)]
    ]
    set active (patch-set active ([neighbors4] of active)) with [sim-grains > 3]
  ]
  report fired
end
...
to update-grains
  set grains sim-grains
end

to reset-sim-grains
  set sim-grains grains
end

to color-patches-by-option
  if color-by = "grains" [ask patches [color-grains]]
  if color-by = "avalanche size" [ask patches [color-avalanche]]
  if color-by = "both" [ask patches [color-both]]
end
...
to color-grains
  ifelse grains <= 3
  [ set pcolor item grains [black green yellow red] ]
  [ set pcolor white ]
end
@#$#@#$#@
GRAPHICS-WINDOW
